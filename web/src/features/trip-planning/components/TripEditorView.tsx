...
import { getTrip, moveStop, removeStop, reorderStops, updateStop } from "@/features/trip-planning/api/trips";
...
export default function TripEditorView({ tripId }: { tripId: string }) {
  ...
  return <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-10"><Link href="/trips" className="text-sm font-bold text-primary hover:underline">← Chuyến đi của bạn</Link><div className="mt-5 flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-primary">{trip.role === "OWNER" ? "Chủ chuyến đi" : canEdit ? "Thành viên có thể chỉnh sửa" : "Thành viên chỉ xem"}</p><h1 className="mt-1 text-h1 text-gray-900">{trip.title}</h1><p className="mt-2 text-gray-600">{trip.description || "Chưa có mô tả cho chuyến đi này."}</p></div>{canEdit && <span className="rounded-full bg-secondary/10 px-3 py-2 text-sm font-bold text-secondary">Bạn có thể chỉnh sửa lịch trình</span>}</div><div className="mt-8 grid gap-6 lg:mt-10 lg:grid-cols-[16rem_minmax(0,1fr)]"><nav aria-label="Các ngày trong chuyến đi" className="flex gap-2 overflow-x-auto rounded-2xl border border-gray-200 bg-white p-3 lg:block">{trip.days.map((day, index) => <button type="button" key={day.id} aria-pressed={index === dayIndex} onClick={() => setDayIndex(index)} className={`mb-1 shrink-0 rounded-xl px-4 py-3 text-left text-sm font-bold lg:w-full ${index === dayIndex ? "bg-primary text-white" : "text-gray-700 hover:bg-gray-100"}`}>Ngày {day.dayIndex}<span className="ml-2 font-medium opacity-80">{day.date}</span></button>)}</nav><section className="min-w-0 rounded-2xl border border-gray-200 bg-white p-4 sm:p-6"><h2 className="text-xl font-bold text-gray-900">Ngày {activeDay.dayIndex} · {activeDay.date}</h2>{mutationError && <p role="alert" className="mt-3 text-sm text-red-700">{mutationError} <button type="button" onClick={() => void load()} className="font-bold underline">Tải lại</button></p>}{activeDay.stops.length === 0 ? <p className="mt-6 rounded-xl bg-gray-50 p-5 text-sm text-gray-600">Chưa có điểm dừng nào trong ngày này.{canEdit ? " Tìm một địa điểm để bắt đầu lập lịch trình." : ""}</p> : <ol className="mt-6 space-y-3">{activeDay.stops.map((stop, index) => <StopCard key={stop.id} stop={stop} editable={canEdit} onRefresh={load} controls={<><button type="button" aria-label={`Đưa ${stop.name} lên`} disabled={index === 0} onClick={() => void reorder(stop.id, -1)} className="rounded border px-3 py-2 text-xs font-bold disabled:opacity-40">Lên</button><button type="button" aria-label={`Đưa ${stop.name} xuống`} disabled={index === activeDay.stops.length - 1} onClick={() => void reorder(stop.id, 1)} className="rounded border px-3 py-2 text-xs font-bold disabled:opacity-40">Xuống</button>{trip.days.filter((day) => day.id !== activeDay.id).map((day) => <button type="button" aria-label={`Chuyển ${stop.name} sang ngày ${day.dayIndex}`} key={day.id} onClick={() => void move(stop.id, day.id)} className="rounded border px-3 py-2 text-xs font-bold">Chuyển ngày {day.dayIndex}</button>)}<button type="button" aria-label={`Xóa ${stop.name}`} onClick={() => void remove(stop.id)} className="rounded border border-red-200 px-3 py-2 text-xs font-bold text-red-700 hover:bg-red-50">Xóa</button></>} />)}</ol>}<div className="mt-6"><RouteMap stops={activeDay.stops} /></div>{canEdit && <PlaceSearch tripId={trip.id} dayId={activeDay.id} stopCount={activeDay.stops.length} onAdded={() => void load()} />}</section></div>{trip.role === "OWNER" && <CollaborationPanel tripId={trip.id} />}</main>;
}

function StopCard({ stop, editable, controls, onRefresh }: { stop: TripDetail["days"][number]["stops"][number]; editable: boolean; controls: ReactNode; onRefresh: () => Promise<void> }) {
  const { tripId } = useParams<{ tripId: string }>();
  const [notes, setNotes] = useState(stop.notes ?? "");
  const [status, setStatus] = useState<"saved" | "saving" | "failed">("saved");
  useEffect(() => {
    if (!editable || notes === (stop.notes ?? "")) return;
    const timer = window.setTimeout(async () => {
      setStatus("saving");
      try { await updateStop(tripId, { ...stop, notes: notes || null }); setStatus("saved"); await onRefresh(); }
