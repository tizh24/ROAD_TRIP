...
function StopCard({ stop, editable, controls, onRefresh }: { stop: TripDetail["days"][number]["stops"][number]; editable: boolean; controls: ReactNode; onRefresh: () => Promise<void> }) {
  ...
  return <li className="rounded-xl border border-gray-100 p-4"><span className="mr-3 inline-flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary">{stop.stopIndex}</span><strong>{stop.name}</strong><p className="ml-10 mt-1 text-sm text-gray-600">{stop.address}</p>{editable ? <label className="ml-10 mt-3 block text-sm font-medium text-gray-700">Ghi chú<textarea value={notes} onChange={(event) => setNotes(event.target.value)} className="field mt-1 min-h-20 bg-white" aria-describedby={`${stop.id}-save`} /></label> : stop.notes && <p className="ml-10 mt-2 text-sm text-gray-700">{stop.notes}</p>}<p id={`${stop.id}-save`} className={`ml-10 mt-2 text-xs ${status === "failed" ? "text-red-700" : "text-gray-500"}`}>{status === "saving" ? "Đang lưu…" : status === "failed" ? "Không thể lưu. Chỉnh sửa vẫn được giữ ở đây; hãy thử lại hoặc tải lại." : "Đã lưu"}</p>{editable && <div className="ml-10 mt-3 flex flex-wrap gap-2">{controls}</div>}</li>;
}
