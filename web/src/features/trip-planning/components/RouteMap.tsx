...
export default function RouteMap({ stops }: { stops: readonly Stop[] }) {
  ...
  useEffect(() => { if (points.length < 2) { queueMicrotask(() => { setRoute([]); setSummary(undefined); }); return; } let active = true; const routeKey = stops.map((stop) => stop.id).join(":"); previewRoute(stops.map(({ latitude, longitude }) => ({ latitude, longitude }))).then((result) => { if (active) { setRoute(result.geometry.coordinates.map(([lng, lat]) => [lat, lng])); setSummary(`${Math.round(result.distanceMeters / 1000)} km · ${Math.round(result.durationSeconds / 60)} phút · ${result.source === "cache" ? "từ bộ nhớ đệm" : "từ nhà cung cấp"}`); trackAnalytics("route_calculated", { outcome: "success", stopCount: stops.length, source: result.source }, { dedupeKey: `${routeKey}:success` }); } }).catch(() => { if (active) { setSummary("Không thể tải tuyến đường; itinerary vẫn được lưu."); trackAnalytics("route_calculated", { outcome: "failure", stopCount: stops.length }, { dedupeKey: `${routeKey}:failure` }); } }); return () => { active = false; }; }, [stops, points.length]);
